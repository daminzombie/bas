...
Fast **`POST /challenge`** service: downloads `video_url`, runs **`custom-ballspotting`** (ball action spotting), returns per-frame-ish predictions (`frame`, `action`, `confidence`) and **`processing_time`**.
...
Each spotting result from `custom-ballspotting` includes at least **`label`**, **`position`** (ms), **`gameTime`**, and **`confidence`** (action softmax at the decoded peak).

Postprocessing uses a compact row type:

```text
PredictionRow = (frame, action, confidence, timestamp_ms)
```

The **`/challenge`** response still exposes only **`frame`**, **`action`**, and **`confidence`**. The raw debug payload aligns with the same three fields plus **`timestamp_ms`**.
